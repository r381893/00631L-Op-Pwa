...
- 履約價：從商品名稱中提取數字（如「台指權28550」的履約價是 28550）
- 權利金：使用「成交均價」欄位的數字
- 口數：使用「總口數」欄位的數字

重要：複式單/價差單的處理
- 複式單會有多列共用同一個「總口數」欄位（合併儲存格）
- 請將複式單拆成獨立的兩筆選擇權部位
- 每一筆部位都要填入相同的口數（從合併的「總口數」欄位讀取）
- 例如：兩列共用「總口數=4」，則兩筆部位都填口數=4

注意事項：
- 商品名稱結尾的 C 代表 Call，P 代表 Put
- 請仔細辨識圖片中的每一筆交易部位
...
option,buy,put,28550,43.6666,3
option,buy,call,29000,29,4
option,sell,call,28900,53,4`;
